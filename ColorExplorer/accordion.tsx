...
import { Volume2, Star, RotateCcw, Play, Check, X, Share2, Palette, Hash } from 'lucide-react';
        ...
        <div className="w-full max-w-6xl flex flex-col items-center space-y-2 h-full">
          <div className="w-full flex justify-between items-center bg-white/40 backdrop-blur-md p-3 rounded-2xl shadow-sm">
            <div className="flex flex-col space-y-1 w-1/4">
              <span className="font-headline font-bold text-xs md:text-sm whitespace-nowrap">Ronda {round + 1} / {shuffledItems.length}</span>
              <Progress value={((round + 1) / shuffledItems.length) * 100} className="h-2 bg-white/50" />
            </div>
